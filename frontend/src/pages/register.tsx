...
import { Loader2, Eye, EyeOff, User, Lock, Globe, ArrowRight } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 4) {
      setError(lang === "ar" ? "كلمة المرور قصيرة جداً" : "Password is too short");
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden" style={bgStyle}>
      ...
      <button
        onClick={goHome}
        className="absolute top-4 md:top-5 left-4 md:left-5 flex items-center gap-2 px-3 md:px-4 py-2 rounded-xl text-xs md:text-sm font-medium text-white/70 hover:text-white transition-all z-20 cursor-pointer"
        style={langBtnStyle}
      >
      ...
      <div className="w-full max-w-[400px] relative z-10" dir={lang === "ar" ? "rtl" : "ltr"}>
        <div className="rounded-3xl p-6 md:p-8 animate-scaleIn" style={cardStyle}>
          ...
          <div className="text-center mb-6">
            <h2 className="text-[22px] md:text-[24px] font-bold text-white mb-1">{lang === "ar" ? "إنشاء حساب جديد" : "Create Account"}</h2>
            <p className="text-zinc-400 text-[12px] md:text-[13px]">{lang === "ar" ? "ابدأ رحلتك مع Server Hub" : "Start your journey with Server Hub"}</p>
          </div>
          ...
          <p className="text-center text-zinc-500 text-[13px] mt-5">
            {lang === "ar" ? "لديك حساب بالفعل؟" : "Already have an account?"}{" "}
            <button onClick={goLogin} className="text-purple-400 hover:text-purple-300 font-semibold transition-colors cursor-pointer">
